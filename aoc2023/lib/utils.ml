...
let rec list_group_into_tuples = function
  ...
  | _ ->
      failwith
        "Utils.list_group_into_tuples: List has an odd number of elements"

let list_to_triple = function
  | [ a; b; c ] -> (a, b, c)
  | _ ->
      failwith "Utils.list_to_triple: List does not have exactly three elements"
...
let elem_indices_of_list ~elem ~compare lst =
  let rec aux acc index = function
    | [] -> List.rev acc
    | hd :: tl ->
        if compare hd elem = 0 then aux (index :: acc) (index + 1) tl
